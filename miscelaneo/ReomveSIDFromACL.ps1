		...
		{ 
	    	...
	    	{		
		 		...
		 		ForEach ($folder in $folders)
				{
				   ...
				   write-host "Procesando $PSPath"
				   try {
		   		     DeleteSID($PSPath)
				   } catch {
				     write-error "Error calling DeleteSID on $PSPath"
				   }
				}
			}
			else
  	...
  	try
  	{
   		...
   		foreach($acc in $acl.access )
   		{
			  ...
			  try {
			    ...
			    write-host "Error removing SID $value on $Path"
				#write-host "$error"
			  }
   			}
   		}
...
Remove-OSCSID -Path "d:\tq1" -Recurse
# si por algun motivo falla que no puede limpiar los SID fantasma
# deshabilitar la herencia
# y reasignar el propietario de todo el d:\tq1
# estos comandos estan en generarAclsZeus initfs
